...
def _try_llm_extractions(
    ...
    def _sleep_backoff(iter_idx: int) -> None:
        sleep_for = min(
            (cfg.backoff_base_s * (2 ** iter_idx)) + random.uniform(0, cfg.jitter_s),
            cfg.backoff_max_s,
        )
    ...
    def _should_retry(e: Exception) -> bool:
        if _is_permanent_error(e):
            return False
        return isinstance(e, TimeoutError) or _is_transient_error(e)

    try:
        extractions, tokens, latency_ms, llm_info = _retry_call(
            lambda: _extract_once(text, resolved_cfg), attempts, _sleep_backoff, _should_retry
        )
